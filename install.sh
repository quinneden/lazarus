...
get_host() {
    echo -en "Choose a ${GREEN}host${NORMAL} - [${YELLOW}M${NORMAL}]acmini or [${YELLOW}V${NORMAL}]irtual machine: "
    read -n 1 -r
    echo
    ...
    if [[ $REPLY =~ ^[Mm]$ ]]; then
        ...
        echo "Invalid choice. Please select 'M' for macmini or 'V' for virtual machine."
        exit 1
    fi
