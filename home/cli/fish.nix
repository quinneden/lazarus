{
  ...
  home.packages = [pkgs.fishPlugins.autopair inputs.grc-rs.packages.${pkgs.system}.default];
  programs.zoxide = {
    enable = true;
  ...
  programs.starship = {
    ...
    enableFishIntegration = true;
    settings = {
      format = "$directory$git_branch$git_commit$git_state$git_status$nix_shell$cmd_duration$line_break$character";
      nix_shell = {
  ...
  programs.fish = {
    ...
    shellAliases = {
      ...
      copy = "wl-copy";
      mkdir = "mkdir -p";
      nix-switch = "sudo nixos-rebuild switch --flake ~/.dotfiles#lazarus";
      cdnix = "cd ~/.dotfiles";
    };
    shellInit = ''
      set TTY1 (tty)
